...
$(document).on("blur", "#PortNumber", function(){
		...
		if(val !=''){
			$.ajax({
				...
				beforeSend: function(){
					...
					//root.next().show();
					//interval = setInterval(blink_text, 100);
				},
				complete: function(){
				...
				success: function(data){
					...
					if (data.success == "1") {
						...
						$("#DatabaseName").parent().removeClass("has-error");
						/*$("#DatabaseName").parent().next().parent().removeClass("has-error");
						$("#DatabaseName").parent().next().children().text('');
						...
						//$(data.id).parent().next().parent().addClass("has-error");
						//$(data.id).parent().next().children().text(data.txt);
					}
					var error = false;
					$("input.field").each(function(){
						if($(this).parent().next().parent().hasClass("has-error"))
							error = true;
					});
					if(error)
						$("#install").prop('disabled', true);
					else $("#install").prop('disabled', false);
				}
			});
		}
	});
	...
	//$("#DatabaseUserName").on("blur", function(){
		var val = $.trim($(this).val());
		var root = $(this);
		var interval = null;
		var hostName = $.trim($('#DatabaseHostName').val()), dbUser = $.trim($('#DatabaseUserName').val()),
		...
		if(val != '')
		{
			$.ajax({
					...
					success: function(data){
						...
						$("#DatabaseUserName").parent().removeClass("has-error");
							//$("#DatabaseUserName").parent().next().parent().removeClass("has-error");
							//$("#DatabaseUserName").parent().next().children().text('');
						//	$("#DatabasePassword").parent().next().parent().removeClass("has-error");
						//	$("#DatabasePassword").parent().next().children().text('');
						}else{
							//if hostname or database name is invalid
							...
							//$("#DatabaseUserName").parent().next().parent().addClass("has-error");
							//$("#DatabasePassword").parent().next().parent().addClass("has-error");
						//	$("#DatabaseUserName").parent().next().children().text('Database credential is invalid');
							//$("#DatabasePassword").parent().next().children().text('Database credential is invalid');
						}
					}
				});
		}
	...
	//$("#DatabasePassword").on("blur", function(){
		var val = $.trim($(this).val());
		var root = $(this);
		var interval = null;
		var hostName = $.trim($('#DatabaseHostName').val()), dbUser = $.trim($('#DatabaseUserName').val()),
